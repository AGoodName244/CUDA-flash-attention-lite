...
from .flash_attn_ops import flash_attn_split, flash_attn_online
import os
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # =================== Modified Version ======================
        xq = xq.transpose(1, 2).contiguous()      # [B, H, Tq, D]
        keys = keys.transpose(1, 2).contiguous()  # [B, H, Tkv, D]
        values = values.transpose(1, 2).contiguous()

        B, H, Tq, D = xq.shape
        Tkv = keys.shape[2]

        env_flag = os.getenv("LLAMA_USE_CUSTOM_ATTN", "1")
        enable_custom = (env_flag == "1")
        enable_custom = True
        ...
        if use_custom:
            is_prefill = (start_pos == 0 and Tq == Tkv)

            # if is_prefill and Tq <= 128:
            #     out = flash_attn_online(xq, keys, values, is_causal=True)
            #     # out = flash_attn_split(xq, keys, values, is_causal=True)
            # elif is_prefill:
            #     # out = flash_attn_split(xq, keys, values, is_causal=True)
            if is_prefill:
                out = flash_attn_online(xq, keys, values, is_causal=True)
                ...
                # out = flash_attn_split(xq, keys, values, is_causal=False)
                out = flash_attn_online(xq, keys, values, is_causal=False)
                # scores = torch.matmul(xq, keys.transpose(2, 3)) / math.sqrt(self.head_dim)
                # if mask is not None:
                #     scores = scores + mask
                # scores = F.softmax(scores.float(), dim=-1).type_as(xq)
